...
- **2026-09-21 — Iteration U — clôture + release v1.13.0** (`chore(iter-U/phase-6.1)` +
  `feat(v1.13.0)` + `docs`). Retrait de l'article de test du renderer
  (`_renderer-test.md`, section TOC « Interne ») et de son SVG dédié
  (`public/docs/sinusoide-annotee.svg`, sans autre consommateur). Bump
  `package.json` 1.12.0 → **1.13.0**. Contexte scindé mis à jour (roadmap U déplacée
  ici, section « Itération terminée : U »). La release embarque aussi les correctifs
  post-U ci-dessous.
- **2026-09-21 — Correctifs post-U (quick wins / bugs connus)** (`fix(post-U/qw.1→qw.5)`),
  menés en parallèle par sous-agents en worktrees isolés, relus puis intégrés
  linéairement sur `main`. **Non validés visuellement en navigateur à la livraison.**
  - **qw.1 — Bibliothèque écrasée sous 900 px** : la règle `@media (max-width: 900px)`
    de `PatchBank.css` (bandeau `max-height: 180px`, héritée de l'iter A quand la
    banque était une sidebar) était du **code mort nuisible** — `PatchBank` n'est plus
    monté que dans l'onglet plein écran (`App.jsx`, `library-tab-content`), les
    sidebars/popovers montent `PatchPicker`/`RecentPatchesList`. Règle **supprimée**
    (pas scopée).
  - **qw.2 — icônes undo/redo du Composer** : `⟲`/`⟳` Unicode → Lucide `Undo2`/`Redo2`
    (16 px), `.history-btn` en `padding: 0` (le padding UA ne laissait que ~14 px).
  - **qw.3 — table `lib/shortcuts.js` réalignée sur les handlers** : ajout de
    `global-tour` (Ctrl+J, absent) et `designer-spline-remove-anchor` (Suppr, sans
    ancre → page Raccourcis seule) ; descriptions corrigées (`composer-paste`,
    `composer-duration-base` 1 = carrée, `composer-duration-coef`, `composer-merge`,
    `library-paste`, `library-rename`, `designer-notes`) ; noms d'onglets tirés de
    `STRINGS.tabs.*`. Aucun handler modifié.
  - **qw.4 — Ctrl+D ouvrait les favoris du navigateur** : cause démontrée — le
    `preventDefault` n'était atteint ni hors onglet Composition, ni sans clip
    sélectionné, ni avec le focus resté sur un `select`/slider. Ctrl+D (split ÷2) et
    Ctrl+Shift+D (split ÷3) sont désormais neutralisés en tête du handler, hors champ
    de saisie texte. (Le backlog parlait à tort de « dupliquer » et de
    `NOTE_GUARD_KEYS`.)
  - **qw.5 — micro-sauts au dezoom molette (Timeline)** : le recalage de `scrollLeft`
    passait par un `requestAnimationFrame` non ordonné par rapport au commit React
    (`zoomH` vit dans le reducer global) → une frame peinte à côté, ou cible clampée
    par l'ancien `scrollWidth`. Cible absolue calculée dans le handler, stockée dans
    `pendingZoomScrollRef {zoom, scrollLeft}`, appliquée en `useLayoutEffect([zoomH])`
    (même commit que la largeur). Même correction pour le zoom rectangle Alt+drag.
    Race latente depuis l'iter A ; le lien avec L.1 reste une supposition.
- **2026-06-15 — Iteration U — phase U.5 (Tour guidé de la Création à jour)**
  (`feat(iter-U/phase-5.1→5.3)` + doc). le Tour Ctrl+J
...
- **2026-06-14 — Iteration U — phase U.4 (peuplement des contenus, domaine writer)**
  (`docs(iter-U/phase-4-redaction)`). Prose pédagogique des 7 articles `creation-*`
  (retrait des lignes « Version provisoire »), schémas SVG dans `public/docs/`
  (enveloppe AHDSR, réponses de filtre, transfert de distorsion, barres d'harmoniques,
  forme d'onde de référence, oscillation LFO), et **couture de `guide-designer.md`**
  en porte d'entrée vers les articles par module. Ids `{#id}` inchangés (contrat U.1 :
  les ids survivent aux reformulations).
...
- **2026-06-12 — Iteration T — clôture + release v1.12.0** (`feat(v1.12.0)` + `docs`).
  ...
  `__APP_VERSION__` lu dynamiquement de `package.json` par Vite → un seul point à bumper).
  Contexte rebrassé : TL;DR + `## État actuel` passés à « entre deux itérations » ; section
  roadmap T retirée de `CONTEXT.md` et **déplacée** vers « Roadmaps des itérations closes
  (B→T) » de l'archive ; saga « ## Itération terminée : T » ajoutée ; **décisions
  architecturales de T consolidées sur place** (helper par lib audio/graphe « ce qu'on voit =
  ce qu'on entend », insertion conditionnelle des nœuds par l'appelant, `ParamEnv` généralisé,
  règle `.osa` absent→défaut complétant le strict-strict).
- **2026-06-12 — Iteration T — T.6quinquies (phase-6.9) : drive continu + point de distance
  max en replié** (`feat(iter-T/phase-6.9)` + doc). Affinage de la poignée 2D (T.6quater) sur
  deux retours d'usage. **(1) Drive float** dans les 3 modes : l'arrondi entier (héritage du
  stepper, sans fondement — `k` est continu dans les formules de transfert) supprimé au reducer
  (`SET_EDITOR_DISTORTION_POINT` brut), au drag (`distortionDriveForX` écrit le float) et à
  l'affichage (`NumberInput` arrondi à 0.1 ; steppers ±1/±5 inchangés). Cache `(curve, drive)`,
  `clampToRange` et `isNumberInRange` (.osa) acceptaient déjà le float → round-trip d'un drive
  décimal (ex. 7.3) préservé. **(2) Mode replié** : nouvelle abscisse caractéristique
  **`x_c=(2/kπ)·arccos(2/kπ)`** (point où `sin(kπx/2)−x` est maximal, lieu d'écart max à la
  diagonale) remplaçant le 1ᵉʳ sommet `1/k` — à drive 1, x_c≈0.561 (écart 0.21) au lieu de
  converger vers l'angle (1,1) : **plus de poignée morte/demi-morte** ; à drive élevé, x_c→1/k
  (continuité avec l'ancien comportement). Inversion par **dichotomie** (`bisectDrive` extrait et
  partagé avec la douce, x_c strictement décroissant). Poignée posée **sur la courbe effective**
  `y=mix·f(x_c)+(1−mix)·x_c` (fold) ; dénominateur `f(x_c)−x_c` jamais nul → la garde résiduelle ne
  protège plus que `hard` à drive→1. Dure/douce inchangées.
...
- **2026-06-08 — Iteration R « Refonte petit écran du Designer » — CLOSE. Release
  ...
  **sous le plancher accordéon** (< 924×668), où l'on manque de largeur ET de hauteur.
  Livré : switcher de modules (un module plein cadre) + contrôles de header relogés
  (R.1/R.1.3), top header priority-plus + hamburger (R.2), gate orientation-aware
  300/500 + densification < 700×500 (R.3), correctifs petit écran (R.3.rectif `.1`→`.7`).
  **R.4 (orientation adaptative) ANNULÉE** → backlog « repenser écrans minuscules ».
  Reste = **confort tactile** (Pointer Events + PWA) → itération S. Phases : **R.1**
  ...
  - **R.3.rectif — Correctifs petit écran (< 924×668)** (`fix(iter-R/phase-3.rectif.1)`
    ...
    - **rectif.7 — masquer « Égaliser » en intermédiaire 924–1100**
...
## Roadmaps des itérations closes (B→U)
...
### Itération R (Refonte petit écran du Designer) — clôturée 2026-06-08 (v1.10.0)

Rendre le Designer utilisable **sous le plancher accordéon** (< 924×668), où l'on
manque de largeur ET de hauteur. Détail par-phase (texte exhaustif) dans l'Historique
ci-dessus. **R.4 (orientation adaptative) annulée** → backlog. Ci-dessous les
paragraphes de synthèse (ex-TL;DR du brief).

- ✅ **R.1 — Switcher de modules** : le petit écran abandonne l'**accordéon** (barres
  repliées qui mangeaient la hauteur) au profit d'un **switcher** : une rangée d'icônes
  de module dans la `DesignerToolbar` (`.designer-module-switcher`, mobile only)
  sélectionne **un seul module affiché plein cadre** ; toute la hauteur va au module
  ouvert. Les 6 corps restent **montés en permanence** (contrainte canvas/RO — masqués
  en `display:none`, l'actif en `display:flex`). Nouveau champ persisté
  **`designerMobileModule`** (∈ les 6 ids, défaut `'canvas'`, validé à l'hydratation) —
  remplace l'ex-volatile/nullable `mobileExpandedZone` ; **toujours exactement un module
  actif** (clic sur l'actif = no-op). Gating rAF de la mini-courbe LFO rebranché sur
  `designerMobileModule === 'modulation'`.
- ✅ **R.1.3 — Contrôles de header relogés** (correctif) : R.1 masquait les **headers
  internes** des modules → leurs contrôles (toggle Libre/Ancres, cap, live/dB/peak, etc.)
  disparaissaient. Désormais les **contrôles de header du module actif** sont **relogés
  dans la `DesignerToolbar`**, après le switcher, dans un **2ᵉ `OverflowToolbar`** —
  trop-plein dans un `…` ; aucun groupe si le module actif n'a pas de contrôle.
  Disposition : `[patch · Presets · Reset] ┊ [switcher 6 icônes] … [contrôles module
  actif]` — switcher **ancré à gauche** (immobile). Items exposés comme **donnée** (pas
  de double rendu) : children-API `moduleHeaderItems` + helper `buildSpectrogramHeaderItems` ;
  en mobile les headers in-body rendent `[]`.
- ✅ **R.2 — Top header priority-plus + hamburger** : le `Tabs` passe en priority-plus
  sous 924×668 — titre tronquable à gauche, `OverflowToolbar` qui reloge onglets +
  auxiliaires, trop-plein → **hamburger** (`Menu`). Ordre de priorité : Création ·
  Composition · Bibliothèque · Documentation · thème · raccourcis · visite (l'ordre
  visuel compact mène par Création/Composition, ≠ desktop). `OverflowToolbar` gagne
  `triggerIcon`. Ancre Tour `header-tabs-zone` préservée. Limite connue : la ghost row
  duplique les `data-anchor` aux → overlay Ctrl+K dégrade son ancrage (non bloquant).
- ✅ **R.3 — Réouverture basse résolution + densification** : le gate (`TooSmallGate`,
  ex-`ResolutionGate`) passe d'un plancher fixe 700×500 à un plancher **orientation-aware
  300/500** (côté court ≥ 300 ET côté long ≥ 500 ; `MIN_USABLE_SHORT`/`MIN_USABLE_LONG`).
  Passe de **densification CSS** bornée stricte à `width < 700 OU height < 500` —
  paddings/gaps réduits, textes/onglets resserrés, modales capées au viewport ; rien ne
  change ≥ 700×500. Limitation **assumée** : tactile fine sous ~700 inadaptée (cas
  d'usage = souris/fenêtre desktop rétrécie).
- ✅ **R.3.rectif — correctifs petit écran** (`.1`→`.7`) : (1) header compact aminci +
  version inline→tiroir ; (2) marge gauche du layout mobile → 0 ; (3) Instrument mobile
  aligné sur l'intermédiaire desktop (système `[⚙]` + octave stepper dans le header) ;
  (4) DesignerToolbar mobile `flex-wrap` (patch/Presets/Effacer/switcher jamais masqués) ;
  (5) fix débordement module Modulation à hauteur rare ; (6) `closeOnSelect` (tiroir
  hamburger se ferme à la sélection) ; (7) « Égaliser » masqué en intermédiaire 924–1100.

**Annulé / reporté** : **R.4 (orientation adaptative des toolbars)** — réorientation
...
### Itération T (effets sans mémoire) — clôturée 2026-06-12 (v1.12.0)

Tour complet des effets qui s'intègrent au **cycle de vie audio jetable** (chaîne par note,
zéro queue) avant le futur chantier des effets à mémoire. Tout **par patch**, `.osa` **v4**
(bump unique en T.2, défauts injectés). Distorsion **par voix**. Aboutit à **9 effets** dans
le module Effets. Cadrage : `archi/BACKLOG.md` (« Effets et modulations »).

- ✅ **T.1 — refonte module « Effets » + switcher header (UI pure)** : module Modulation
  renommé « Effets » (label seul, id `'modulation'` inchangé) ; corps **un effet à la fois**
  (l'autre monté/masqué, rAF sur le seul graphe visible) ; rangée de boutons toggle en barre
  de titre (`OverflowToolbar`, highlight = en édition, pastille accent = activé, clic = édition
  seule, badge agrégé tiroir via `triggerBadge`) ; `buildEffectsHeaderItems` partagé
  desktop/mobile ; état UI `designerEffectsSelected` persisté hors undo. Aucun changement audio.
- ✅ **T.2 — auto-pan (LFO → `StereoPannerNode.pan`, `.osa` v4)** : 1ᵉʳ effet **stéréo**.
  `autoPan: Lfo` (depth 0..1 = excursion symétrique G↔D) ; panner inséré par l'appelant
  **seulement si `enabled && depth>0`** (mono inchangé sinon), 4 chemins ; 3ᵉ bouton + panneau
  (graphe à étiquettes G/D). **`OSA_VERSION = 4`** — seul bump de l'itération (accepte v1→v4).
- ✅ **T.3 — pitch envelope (→ `osc.detune`, v4 inchangé)** : 1ʳᵉ modulation **non-LFO**.
  `pitchEnv {enabled, amount cents signé, time ms}` ; automation de la valeur de base
  d'`osc.detune` (**aucun nœud** ; somme avec le vibrato par construction) ; 4ᵉ bouton
  « Hauteur » + panneau à 2 poignées sans animation. **T.3bis** : mode **Inverser** (part de
  la note, s'éloigne et y reste) + graphe miroir. **T.3ter** : **4 formes de progression**
  (`curve`, `setValueCurveAtTime` 64 pts — pas `exponentialRamp`, ne traverse pas zéro).
- ✅ **T.4 — filtre statique (`BiquadFilterNode` par voix, v4 inchangé)** : 1ᵉʳ filtre + 1ᵉʳ
  graphe **fréquentiel**. `filter {enabled, type LP/HP/BP/notch, cutoff, q linéaire}` ;
  insertion **VCO→VCF→VCA** conditionnelle ; **mapping Q** dB↔linéaire partagé audio/graphe
  (`lib/filter.js`) ; 5ᵉ bouton + panneau (switch 4 types, steppers Fréquence **multiplicatifs**)
  + graphe de réponse (`getFrequencyResponse`, biquad de mesure) à poignée 2D (`SET_EDITOR_FILTER_POINT`).
  `NumberInput` += `stepFactor`. Keytracking du cutoff → backlog.
- ✅ **T.5 — enveloppe de filtre + wah (→ `biquad.detune`, v4 inchangé)** : `biquad.detune`
  en cents comme l'osc → l'env de filtre **réutilise le pitch env**, le wah **réutilise le
  vibrato** (no-op si filtre off). `PitchEnv` généralisé en **`ParamEnv`** ; `filterEnv: ParamEnv`
  (±4800 cents) + `wah: Lfo` (depth 0..3600). Helper audio partagé **`scheduleParamEnv`**. 2 boutons
  (panneaux clones, graphe généralisé par bornes) + hint « filtre désactivé ». Fix `UPDATE_PATCH`
  persiste enfin `filter` (oubli T.4).
- ✅ **T.6 — distorsion (`WaveShaperNode` 4x par voix, v4 inchangé)** : `distortion {enabled,
  curve soft/hard/fold, drive 1..50, mix}` ; courbes normalisées ±1→±1 (`lib/distortion.js`, mémo
  + helper audio/graphe) ; inséré **AVANT le filtre** (logique soustractive) ; split wet/dry ; 8ᵉ
  bouton + graphe de transfert à poignée Drive. **T.6bis** (disto vivante, 3 sous-commits 6.4–6.6) :
  graphe = courbe **effective** `mix·f(x)+(1−mix)·x` (+ wet pure grisée si mix<1) ; `driveEnv: ParamEnv`
  (3ᵉ instance, `amount` = gain ±1) module un **gain d'entrée du shaper** (`scheduleParamEnv` += `base`,
  la courbe d'un WaveShaper n'étant pas un AudioParam) ; défaut **invert** « ampli » ; 9ᵉ bouton « Env. drive ».
- ✅ **T.6ter→6.9 — rectificatifs de la poignée disto** : **6.7** 2ᵉ poignée Mix (manipulable au
  geste) ; **6.8** remplacée par **UNE poignée 2D** au **point caractéristique** `x_c(curve, drive)`
  (lieu d'écart max à la diagonale ; commit atomique `SET_EDITOR_DISTORTION_POINT`) ; **6.9** drive
  **float continu** (fin de l'arrondi entier) + nouvelle abscisse fold `x_c=(2/kπ)·arccos(2/kπ)` (point
  de distance max, inversée par dichotomie partagée avec la douce) → fin des poignées mortes.

### Itération U (documentation utilisateur de la Création) — clôturée 2026-09-21 (v1.13.0)
...
- ✅ **U.6 — clôture** : retrait de `_renderer-test.md` (section TOC « Interne ») et de
  son SVG de démo ; release v1.13.0.

## Itération terminée : R — Refonte petit écran du Designer

**Déclencheur** : sous le plancher accordéon (< 924×668), le Designer était
inutilisable — on y manque de **largeur ET de hauteur**, et l'ancien accordéon
(modules empilés en barres repliées) gaspillait justement la hauteur. Il fallait
repenser la disposition petit écran, pas seulement la rétrécir.

**Livré** : un **switcher de modules** (un seul module plein cadre, sélectionné dans
la toolbar, les 6 corps montés en permanence) qui rend toute la hauteur au module
ouvert ; les **contrôles de header** des modules, masqués par le switcher, **relogés**
dans la toolbar (2ᵉ OverflowToolbar) ; le **top header** en priority-plus avec
**hamburger** d'en-tête (largeur) ; un **gate de résolution abaissé** orientation-aware
**300/500** (300×500 portrait ET 500×300 paysage passent) doublé d'une **densification
CSS** < 700×500 ; puis une passe de **correctifs ciblés** (R.3.rectif `.1`→`.7` : header
aminci + version toujours visible, marge gauche → 0, Instrument mobile aligné sur
l'intermédiaire desktop, toolbar wrappable, fix débordement Modulation, fermeture du
tiroir hamburger, masquage d'« Égaliser » en intermédiaire). **Desktop ≥ 924×668
strictement inchangé** tout du long.

**Abandonné** : **R.4 (orientation adaptative)** — réorienter les toolbars row/column
selon le ratio pour récupérer de la hauteur en paysage. Tentée puis **annulée en bloc**
(trop de casse pour un résultat médiocre). La **refonte conceptuelle des écrans
minuscules** est renvoyée au **backlog** (« repenser le concept »).

**Reste** : le **confort tactile** sous ~700 px (dessin au doigt, poignées) — limitation
assumée de R (cas d'usage visé = souris, fenêtre desktop rétrécie). Cap pressenti :
**itération S** (Pointer Events, agrandissement des surfaces tactiles, PWA).
...
## Itération terminée : T — Effets sans mémoire

**Cadrage** : après le support tactile (S), le synthé avait un timbre (Designer) mais
peu de **caractère sonore**. Plutôt que d'ouvrir tout de suite le gros chantier des
effets **à mémoire** (delay, reverb, chorus — qui exigent des nœuds persistants et des
queues, en rupture avec l'archi « une chaîne jetable par note »), on a fait d'abord le
**tour complet de ce qui s'intègre sans rien changer au cycle de vie audio** : tout
effet calculable dans la voix éphémère, par patch. Le pari : épuiser le « sans mémoire »
donne déjà une palette riche, et clarifie ce qui restera vraiment à faire côté mémoire.

**Livré** : **9 effets par patch** dans un module « Effets » à switcher (un panneau
plein cadre à la fois, boutons toggle en barre de titre). Dans l'ordre des phases —
**auto-pan** (1ᵉʳ effet stéréo, `StereoPannerNode`, le seul bump `.osa` v4 de
l'itération) ; **pitch envelope** (1ʳᵉ modulation non-LFO, automation de la base
d'`osc.detune`, + mode Inverser + 4 formes de progression) ; **filtre statique**
(`BiquadFilterNode` par voix, 1ᵉʳ graphe fréquentiel à poignée 2D) ; **enveloppe de
filtre + wah** (sur `biquad.detune`) ; **distorsion** (`WaveShaperNode` 4x par voix,
3 courbes, mix wet/dry) ; **enveloppe de drive**. Le tout joué sur les **4 chemins de
synthèse** via le helper partagé `applyModulation`, chaque effet **inséré
conditionnellement** par l'appelant (chaîne bit-identique quand l'effet est off).

**Trois leçons d'archi qui ont structuré l'itération** :
- **Le détune en cents est une devise commune.** Découvrir que `biquad.detune` est en
  cents *comme* `osc.detune` a tout débloqué : l'enveloppe de filtre **réutilise** le
  pitch env, le wah **réutilise** le vibrato — sans code neuf, juste en pointant la même
  automation sur un autre `AudioParam`. D'où la généralisation `PitchEnv → ParamEnv` et
  le helper unique `scheduleParamEnv(param, env, start, base)` (le `base` permettant même
  de moduler un **gain** d'entrée quand la cible n'est pas un param — la courbe d'un
  WaveShaper n'en étant pas un).
- **« Ce qu'on voit = ce qu'on entend » se garantit par construction, pas par
  discipline.** Chaque lib feuille (`filter.js`, `distortion.js`, `pitchProgression`)
  expose une seule fonction de calcul, appelée à l'identique par l'audio et par le
  graphe. Le graphe de filtre interroge un *biquad de mesure* avec le même mapping Q ; le
  graphe de disto trace la même `distortionTransfer` que la table du shaper. Zéro
  réimplémentation parallèle = zéro dérive son/dessin.
- **Une poignée doit vivre là où l'effet se voit.** La disto a coûté le plus de
  rectificatifs (T.6ter→6.9) : poignée Drive verticale (drag ≠ effet visible), puis
  poignée Mix (cas mort au croisement `f(x₀)=x₀`), puis **une** poignée 2D posée au
  **point caractéristique** `x_c(curve, drive)` — le lieu d'écart maximal à la diagonale,
  donc l'endroit où la non-linéarité se lit le mieux. Dernier raffinage (6.9) : **drive
  continu** (l'arrondi entier n'avait aucun fondement — `k` est continu dans les formules)
  et, pour le mode replié, une abscisse `x_c=(2/kπ)·arccos(2/kπ)` (le vrai point de
  distance max au lieu du premier sommet `1/k`) qui supprime les configurations de poignée
  morte à bas drive. Inversion par dichotomie, mutualisée avec la douce.

**Hors scope (→ futur)** : distorsion « sur la somme » par patch, LFO sur le drive, ring
mod / FM, pitch « fall » au release, keytracking du cutoff — tous notés au backlog
« inattendus ». Le gros morceau **effets à mémoire** reste devant.

**Release** : v1.12.0 (2026-06-12), bump de version seul (toutes les features étaient
déjà sur `main`, livrées au fil des phases). Symboles clés introduits sur l'arc :
`autoPan`/`pitchEnv`/`filter`/`filterEnv`/`wah`/`distortion`/`driveEnv` (modèle),
`ParamEnv` + `scheduleParamEnv`, `lib/filter.js`, `lib/distortion.js`,
`SET_EDITOR_FILTER_POINT`/`SET_EDITOR_DISTORTION_POINT`, `OSA_VERSION = 4`.

## Itération terminée : U — Documentation utilisateur de la Création

**Cadrage** (2026-06-12) : après T, l'onglet Création portait six modules et neuf
effets, mais la documentation en était restée au survol narratif de l'itération L.
Plutôt qu'un manuel de plus, on a voulu une **doc interactive** : depuis l'app, un
clic sur un contrôle mène au paragraphe qui l'explique. Quatre décisions de cadrage
ont tenu jusqu'au bout : granularité = **contrôle logique** (un graphe = un badge, le
paragraphe détaille ses poignées) ; **un article par module** (`creation-*`,
`guide-designer` restant le survol) ; navigation par **bascule d'onglet** (pas de
panneau in-situ) ; prose = **writer**.

**Déroulé** : U.1 a posé le socle côté renderer (ids de titre explicites `{#id}` sans
auto-slug, liens profonds `doc:article#fragment`, `<Details>`, images SVG). U.2 a
bouclé le mode Info de bout en bout sur un périmètre réduit (bouton + Ctrl+I,
`InfoOverlay`, registre déclaratif `DOC_TARGETS` moulé sur `SHORTCUTS`). U.3 a étendu
la couverture — trop large : le **rectificatif U.3.r** a recentré sur la Création seule
(retrait de la Bibliothèque), déplacé les badges Effets sur les boutons du switcher,
centré les pastilles sur leur contrôle et comblé quatre trous. U.3.s a rattrapé les
sous-contrôles des effets via des **sections de concept partagées par famille**
(LFO/Enveloppe/Filtre/Disto), le registre étant **généré par famille** : 94 cibles.
U.4 (writer) a peuplé les articles et recousu `guide-designer`. U.5 a remis le Tour
guidé de la Création à niveau (12 étapes, `revealModule`, pont final vers Ctrl+I).

**Leçons** :
- **Ids explicites plutôt qu'auto-slug** : le contrat entre code (`DOC_TARGETS`) et
  prose (writer) tient parce que reformuler un titre ne casse aucun lien.
- **Documenter le concept, pas l'occurrence** : « Vitesse » existe dans quatre LFO ;
  une section par famille + un registre généré évitent 43 paragraphes quasi
  identiques et leur dérive.
- **Le Tour oriente, l'Info détaille** : deux outils, deux rôles ; le Tour se termine
  en passant la main au mode Info.
- **Squelettes factuels avant la prose** : l'implémenteur fournit headings, ids et
  faits bruts vérifiés (bornes, unités, défauts) ; le writer n'a pas à fouiller le
  code, et la couverture se valide avant que la prose existe.
- **Cadrer le périmètre avant de couvrir** : U.3 a badgé la Bibliothèque par élan ;
  il a fallu un rectificatif pour revenir au sujet de l'itération.
